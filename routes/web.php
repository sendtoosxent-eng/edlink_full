...
Route::post('contact', function (Request $request) {
    $data = $request->validate(['name' => 'required|string|max:255', 'email' => 'required|email|max:255', 'subject' => 'required|string|max:255', 'message' => 'required|string|max:5000', 'type' => 'nullable|in:contact,issue']);
    ContactMessage::create($data);

    return back()->with('contact_status', 'Thank you. The Edlink team will get back to you shortly.');
...
Route::get('dashboard', function () {
    ...
    $currencyCode = SchoolSetting::where(['school_id' => $school->id, 'key' => 'currency'])->value('value') ?: 'UGX';
    ...
    try {
        $attendanceEndDate = request()->filled('attendance_date') ? Carbon::parse((string) request('attendance_date'))->startOfDay() : today();
    } catch (Throwable) {
    ...
    foreach ($weekDays as $day) {
        $dayRecords = $attendanceRecords->filter(fn ($r) => $r->attendance_date->isSameDay($day));
        $attendanceSeries['all']['present'][] = $dayRecords->whereIn('status', ['present', 'late'])->count();
        $attendanceSeries['all']['absent'][] = $dayRecords->where('status', 'absent')->count();
    }
    foreach ($school?->classes ?? [] as $class) {
        $ids = $class->students->pluck('id');
        $attendanceSeries[$class->id] = ['present' => [], 'absent' => []];
        foreach ($weekDays as $day) {
            $records = $attendanceRecords->whereIn('student_id', $ids)->filter(fn ($r) => $r->attendance_date->isSameDay($day));
            $attendanceSeries[$class->id]['present'][] = $records->whereIn('status', ['present', 'late'])->count();
            $attendanceSeries[$class->id]['absent'][] = $records->where('status', 'absent')->count();
        }
    }
    ...
    foreach ($school?->classes ?? [] as $class) {
        $performanceSeries[(string) $class->id] = $buildPerformanceSeries($performance->where('school_class_id', $class->id));
    }
...
Route::middleware(['auth', 'verified', 'branch.context', 'active.user', 'designation.access'])->group(function () {
    ...
    Route::get('homework', Homework::class)->name('homework.index');
    Route::get('homework/assignments/{assignment}/download', function (HomeworkAssignment $assignment) {
        $user = auth()->user();
        ...
        $canView = $assignment->school_id === $user->school_id && (
            ...
            || ($student
                ...
                && StudentSubjectSelectionService::studentTakesSubject($student, $assignment->term_id, $assignment->subject_id))
        );
    ...
    Route::get('homework/submissions/{submission}/download', function (HomeworkSubmission $submission) {
    ...
    Route::get('exams/{exam}/students/{student}/report-card', function (Exam $exam, Student $student) {
        $user = auth()->user();
        abort_unless(
        ...
        $school = $user->school;
        $blocked = SchoolSetting::where(['school_id' => $school->id, 'key' => 'results_fee_clearance_required'])->value('value') === 'enabled' && $student->balance($exam->term) > 0;
        $scales = GradingScale::where('school_id', $school->id)->get();
        $subjects = $exam->papers->filter(fn ($paper) => DB::table('exam_paper_submissions')->where('exam_paper_id', $paper->id)->value('status') === 'approved')->map(function ($paper) use ($student, $scales) {
            $score = (float) (DB::table('exam_marks')->where(['exam_paper_id' => $paper->id, 'student_id' => $student->id])->value('score') ?? 0);
            $percentage = $paper->maximum_score ? round($score / $paper->maximum_score * 100, 2) : 0;
            $grade = $scales->first(fn ($scale) => $percentage >= $scale->minimum_percentage && $percentage <= $scale->maximum_percentage);

            return compact('paper', 'score', 'percentage', 'grade');
        });
    ...
    Route::get('settings/ledger-mappings', Accounting::class)->name('settings.ledger-mappings');
