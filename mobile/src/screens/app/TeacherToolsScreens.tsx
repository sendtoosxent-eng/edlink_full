...
export function AddHomeworkScreen({ token, onBack }: { token: string; onBack: () => void }) {
  const [attachment, setAttachment] = useState<DocumentPicker.DocumentPickerAsset>();
  ...
  const publish = async () => {
    ...
    setBusy(true); setError('');
    try { const body = new FormData(); Object.entries({ school_class_id: assignment.school_class_id, subject_id: assignment.subject_id, title: title.trim(), instructions: instructions.trim(), maximum_score: Number(maximum), due_at: date.toISOString() }).forEach(([key, value]) => body.append(key, String(value))); if (attachment) body.append('attachment', { uri: attachment.uri, name: attachment.name, type: attachment.mimeType ?? 'application/octet-stream' } as unknown as Blob); await api.upload('/homework', token, body); Alert.alert('Homework published', 'Your assigned learners can now see this homework.'); onBack(); }
    catch (e) { setError(message(e)); } finally { setBusy(false); }
  };
  return <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><Back onPress={onBack} /><PageIntro title="Add homework" description="Publish work for an assigned class and subject." />
    ...
    <Pressable onPress={() => void DocumentPicker.getDocumentAsync({ copyToCacheDirectory: true, multiple: false }).then(result => { if (!result.canceled) { if ((result.assets[0].size ?? 0) > 10 * 1024 * 1024) { Alert.alert('File too large', 'Choose a file smaller than 10 MB.'); return; } setAttachment(result.assets[0]); } }).catch(() => setError('Unable to choose attachment.'))}><Text style={styles.backText}>{attachment ? attachment.name : 'Add attachment (up to 10 MB)'}</Text></Pressable>
    {attachment && <Pressable onPress={() => setAttachment(undefined)}><Text style={styles.body}>Remove attachment</Text></Pressable>}
...
type TeacherHomework = { attachment_name?: string; id: number; title: string; instructions: string; due_at: string; maximum_score: number; subject?: { name: string }; school_class?: { name: string }; submissions?: Array<{ attachment_name?: string; id: number; student: { name: string }; answer?: string; score: number | null; feedback?: string; status: string }> };
export function TeacherHomeworkScreen({ token }: { token: string }) {
  ...
  return <ScrollView contentContainerStyle={[styles.content, { paddingBottom: 100 }]} keyboardShouldPersistTaps="handled">
    ...
    {selected ? <><Pressable onPress={() => setSelected(undefined)}><Text style={styles.backText}>← All homework</Text></Pressable><Text style={styles.cardTitle}>{selected.title}</Text><Text style={styles.body}>{selected.instructions}</Text><Text style={styles.body}>Due {new Date(selected.due_at).toLocaleString()} · /{selected.maximum_score}</Text>
      <NativeAttachment token={token} path={`/homework/${selected.id}/attachment`} name={selected.attachment_name} />
