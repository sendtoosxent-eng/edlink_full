...
export function TeacherScheduleScreen({ token, onBack, backLabel = 'Teaching' }: { token: string; onBack: () => void; backLabel?: string }) {
  ...
  if (error) return <ErrorScreen message={error} retry={load} onBack={onBack} />;
  return <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={busy} onRefresh={load} tintColor={colors.primary} />}>
    <Pressable onPress={onBack}><Text style={styles.back}>← {backLabel}</Text></Pressable><Text style={styles.title}>My timetable</Text>
