...
class Homework extends Component
{
    ...
    public string $classId='', $streamId='', $subjectId='', $title='', $instructions='', $dueAt='', $maximumScore='100';
    public $attachment;
    public ?int $selectedAssignmentId=null;
    public string $answer='';
    public array $reviewScores=[], $reviewFeedback=[];
    ...
    private function isStudent(): bool { return Auth::user()->role === 'student'; }
    ...
    {
        $user=Auth::user(); $term=$user->school->currentTerm();
        $query=DB::table('staff_subjects')->where('school_id',$user->school_id)->when($term,fn($q)=>$q->where('term_id',$term->id));
        if (!in_array($user->role,['admin','superadmin','academic_admin'],true)) $query->where('user_id',$user->id);
    ...
    {
        $user=Auth::user(); abort_if($this->isStudent() || $user->role==='parent',403);
        ...
        $this->maximumScore='100'; $this->dueAt=now()->addDays(2)->format('Y-m-d\TH:i');
        session()->flash('status','Homework published to the class.');
    }

    public function selectAssignment(int $id): void
    {
        $assignment=$this->visibleAssignments()->firstWhere('id',$id); abort_unless($assignment,404);
    ...
    private function visibleAssignments()
    {
        ...
        if ($user->role==='parent') return collect();
        if (!in_array($user->role,['admin','superadmin','academic_admin'],true)) $query->where('teacher_id',$user->id);
