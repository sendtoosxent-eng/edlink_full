...
class Reports extends Component
{
    ...
    private function studentQuery($school,$term,array $with=[]){$financialRelations=$term?['enrolments'=>fn($q)=>$q->where('term_id',$term->id),'arrears'=>fn($q)=>$q->where('applied_term_id',$term->id),'postedFeePayments'=>fn($q)=>$q->where('term_id',$term->id)]:[];return Student::with(array_merge($with,$financialRelations))->where('school_id',$school->id)->when($term,fn($q)=>$q->where(fn($scope)=>$scope->whereHas('enrolments',fn($e)=>$e->where('term_id',$term->id))->orWhere(fn($fallback)=>$fallback->whereDoesntHave('enrolments',fn($e)=>$e->where('term_id',$term->id))->where('term_id',$term->id))))->when($this->gender,fn($q)=>$q->where('gender',$this->gender))->when($this->categoryId,fn($q)=>$q->where(fn($scope)=>$scope->where('student_category_id',$this->categoryId)->orWhereHas('enrolments',fn($e)=>$e->where('term_id',$term?->id)->where('student_category_id',$this->categoryId))));}
