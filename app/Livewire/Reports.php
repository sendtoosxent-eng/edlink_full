...
class Reports extends Component
{
    ...
    public function render()
    {
        ...
        $buildReport = function ($school, $term) use ($dateRange) {
            ...
            'expenses' => $this->table(['Date', 'Receipt / voucher', 'Category', 'Description', 'Amount'], $this->pagedRows($dateRange(Expense::posted()->where('school_id',$school->id)->when($term,fn($q)=>$q->where('term_id',$term->id)),'expense_date')->latest('expense_date'), fn($e)=>[$e->expense_date?->format('d M Y'),$e->reference_number ?? '-',$e->category,$e->description,number_format($e->amount,2)]), 'Expenses', $dateRange(Expense::posted()->where('school_id',$school->id)->when($term,fn($q)=>$q->where('term_id',$term->id)),'expense_date')->sum('amount')),
