    ...
    public function store(AttendanceStoreRequest $request)
    {
        ...
        DB::transaction(function()use($data,$user){
            foreach($data['records'] as $record) AttendanceRecord::updateOrCreate(
                ...
                ['school_id'=>$user->school_id,'term_id'=>$user->school->currentTerm()->id,'school_class_id'=>$data['school_class_id'],'subject_id'=>$data['subject_id']??null,'status'=>$record['status'],'recorded_by'=>$user->id]
            );
        });
