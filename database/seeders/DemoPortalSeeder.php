...
class DemoPortalSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $parent = User::updateOrCreate(['school_id' => $school->id, 'email' => 'parent.demo@edlink.test'], ['name' => 'Grace Nakato', 'phone' => '+256700000101', 'role' => 'parent', 'password' => Hash::make('Password123!'), 'email_verified_at' => now()]);
        ...
        $parent->portalStudents()->syncWithoutDetaching([$student->id => ['school_id' => $school->id, 'relationship' => 'Mother']]);
        $learnerLogin->portalStudents()->syncWithoutDetaching([$student->id => ['school_id' => $school->id, 'relationship' => 'Student']]);

        foreach (range(0, 4) as $offset) AttendanceRecord::updateOrCreate(['school_id' => $school->id, 'term_id' => $term->id, 'student_id' => $student->id, 'attendance_date' => now()->subDays($offset)->toDateString()], ['status' => $offset === 1 ? 'absent' : ($offset === 3 ? 'late' : 'present'), 'recorded_by' => null]);
        ...
        $this->command?->info('Portal demo accounts created for Amina Nakato.');
    }
}
