...
function scopedTeacherFixture(): array
{
    ...
    $classDesignation = Designation::create(['school_id' => $school->id, 'name' => 'Class Teacher', 'permissions' => ['academics.subjects', 'exams.marks', 'exams.results']]);
    $subjectDesignation = Designation::create(['school_id' => $school->id, 'name' => 'Subject Teacher', 'permissions' => ['academics.subjects', 'exams.marks', 'exams.results']]);
    ...
    $otherExam = Exam::create(['school_id' => $school->id, 'term_id' => $term->id, 'school_class_id' => $classB->id, 'name' => 'Other Class Midterm']);
    $otherClassMathPaper = ExamPaper::create(['exam_id' => $otherExam->id, 'subject_id' => $math->id, 'maximum_score' => 100, 'weighting' => 1]);
    DB::table('staff_subjects')->insert([
        ['school_id' => $school->id, 'term_id' => $term->id, 'user_id' => $classTeacher->id, 'subject_id' => $math->id, 'school_class_id' => $classB->id, 'created_at' => now(), 'updated_at' => now()],
    ...
    return compact('school', 'term', 'classA', 'classB', 'classTeacher', 'subjectTeacher', 'studentA', 'studentB', 'mathPaper', 'englishPaper', 'otherClassMathPaper');
}
...
it('shows a class teacher every subject in their class and assigned subjects in other classes', function () {
    $data = scopedTeacherFixture();
    DB::table('class_subjects')->insert([
        ['school_id' => $data['school']->id, 'term_id' => $data['term']->id, 'school_class_id' => $data['classA']->id, 'subject_id' => $data['mathPaper']->subject_id, 'created_at' => now(), 'updated_at' => now()],
        ['school_id' => $data['school']->id, 'term_id' => $data['term']->id, 'school_class_id' => $data['classA']->id, 'subject_id' => $data['englishPaper']->subject_id, 'created_at' => now(), 'updated_at' => now()],
    ]);

    Livewire::actingAs($data['classTeacher'])->test(MarksEntry::class)
        ->assertSee('Mathematics')->assertSee('English')
        ->set('paperId', (string) $data['englishPaper']->id)->assertOk()
        ->set('paperId', (string) $data['otherClassMathPaper']->id)->assertOk();
});
...
it('limits a subject teacher to their assigned subject', function () {
    ...
    Livewire::actingAs($data['subjectTeacher'])->test(MarksEntry::class)
        ->set('paperId', (string) $data['otherClassMathPaper']->id)->assertForbidden();
});

it('makes the subject directory read only and scoped for teachers', function () {
    $data = scopedTeacherFixture();
    DB::table('class_subjects')->insert([
        ['school_id' => $data['school']->id, 'term_id' => $data['term']->id, 'school_class_id' => $data['classA']->id, 'subject_id' => $data['mathPaper']->subject_id, 'created_at' => now(), 'updated_at' => now()],
        ['school_id' => $data['school']->id, 'term_id' => $data['term']->id, 'school_class_id' => $data['classA']->id, 'subject_id' => $data['englishPaper']->subject_id, 'created_at' => now(), 'updated_at' => now()],
    ...
    Livewire::actingAs($data['classTeacher'])->test(Subjects::class)
        ->assertSee('My Subjects')->assertSee('Mathematics')->assertSee('English')
        ->assertDontSee('Add New Subject');

    Livewire::actingAs($data['subjectTeacher'])->test(Subjects::class)
        ->assertSee('My Subjects')->assertSee('Mathematics')->assertDontSee('English')
        ->call('addSubject')->assertForbidden();
});
