...
it('seeds a useful cross-module demo experience', function () {
    ...
    expect(DB::table('school_notifications')->where('school_id', $schoolId)->count())->toBeGreaterThanOrEqual(4)
        ...
        ->and(DB::table('school_classes')->where('school_id', $schoolId)->count())->toBe(7)
        ->and(DB::table('subjects')->where('school_id', $schoolId)->count())->toBe(7)
        ->and(DB::table('class_subjects')->where('school_id', $schoolId)->count())->toBe(49)
        ->and(DB::table('student_categories')->where('school_id', $schoolId)->count())->toBe(2)
        ->and(DB::table('fee_structures')->where('school_id', $schoolId)->count())->toBe(14)
        ->and(DB::table('school_classes')->where('school_id', $schoolId)->whereNotNull('class_teacher_user_id')->count())->toBe(7)
        ->and(DB::table('staff_subjects')->where('school_id', $schoolId)->count())->toBe(28)
        ->and(DB::table('students')->where('school_id', $schoolId)->where('status', 'active')->count())->toBe(350)
