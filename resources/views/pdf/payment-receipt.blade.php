...
<table class="details"><tr><td class="reference"><div class="block-title">Payment Reference</div><div class="reference-box">Method: <strong>{{ strtoupper(str_replace('_', ' ', $payment->method)) }}</strong><br>Ref / Slip ID: <strong>{{ $payment->transaction_id ?: $payment->bank_slip_number ?: 'N/A' }}</strong><br>Received By: <strong>{{ $payment->recordedBy?->name ?: 'Accounts Admin' }}</strong></div><div class="notes"><div class="block-title">Terms & Conditions</div>{{ $payment->notes ?: 'This receipt serves as official confirmation of school fees received. Fees once paid are non-refundable.' }}</div></td><td class="summary"><table class="summary-line"><tr><td>Adjusted Term Fee:</td><td>UGX {{ number_format($student->adjustedFeeAmount($term), 2) }}</td></tr></table><table class="total"><tr><td>Total Paid:</td><td>UGX {{ number_format((float) $payment->amount, 2) }}</td></tr></table><table class="balance"><tr><td>Remaining Balance:</td><td>UGX {{ number_format(max(0, (float) $student->balance($term)), 2) }}</td></tr></table></td></tr></table></div>
