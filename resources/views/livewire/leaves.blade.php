<div class="mx-auto max-w-7xl space-y-6">
    <div class="flex flex-wrap items-end justify-between gap-3">
        <div><h1 class="text-2xl font-bold text-slate-900">Leave requests</h1><p class="mt-1 text-sm text-slate-500">Submit, review and retain staff leave decisions for this school.</p></div>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <form wire:submit="requestLeave" class="mt-4 space-y-4">
                <label class="block text-sm font-medium">Staff member<select wire:model="staffId" class="mt-1 w-full rounded-xl border-slate-200"><option value="">Select staff</option>@foreach($staff as $member)<option value="{{ $member->id }}">{{ $member->name }}{{ $member->job_title ? ' — '.$member->job_title : '' }}</option>@endforeach</select></label>
