<div class="space-y-6">
    ...
    @if($tab==='dashboard')
        ...
        <div class="flex flex-wrap gap-3">@if(auth()->user()->hasPermission('accounting.assessments.generate'))<button wire:click="generateFeeAssessments" wire:confirm="Generate controlled tuition assessments for all active enrolments in the current term?" class="rounded-xl bg-yellow-500 px-4 py-3 text-sm font-black text-slate-950">Generate current-term assessments</button>@endif<button wire:click="setTab('journals')" class="rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-black text-slate-800">Review journal approvals</button></div>
    ...
    @if($tab==='accounts')
        ...
        <section>
            <div class="mb-4 border-b border-slate-200 pb-4"><h2 class="text-xl font-black text-slate-900">Chart of accounts</h2><p class="mt-1 text-xs text-slate-500">Browse accounts and subaccounts by financial class, then select one to update it.</p></div>
            <div class="grid min-h-[680px] items-start gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(400px,.9fr)]">
            <div class="overflow-hidden rounded-xl border border-slate-300 bg-white" x-data="{ accountClass: 'asset' }">
                <div class="flex gap-0 overflow-x-auto border-b border-slate-300 bg-slate-50 px-3 pt-3">
                    @foreach($accountClasses as $classKey => [$classLabel, $tone])
                        <button type="button" x-on:click="accountClass = '{{ $classKey }}'" :class="accountClass === '{{ $classKey }}' ? 'border-slate-300 border-b-white bg-white text-slate-900' : 'border-transparent bg-slate-100 text-slate-600 hover:bg-white'" class="-mb-px whitespace-nowrap rounded-t-lg border px-4 py-2.5 text-xs font-bold transition">{{ $classLabel }}</button>
                    @endforeach
                </div>
                <div class="overflow-x-auto">
                    @foreach(array_keys($accountClasses) as $classKey)
                        <div x-show="accountClass === '{{ $classKey }}'" x-cloak class="min-w-[520px] p-5">
                            <button type="button" wire:click="cancelAccountEdit" class="mb-2 flex w-full items-center gap-2 rounded-lg px-2 py-2 text-left text-sm font-black text-slate-900 hover:bg-amber-50"><span class="text-slate-400">▾</span><span># : Chart of accounts</span></button>
                            <div class="mb-1 flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2.5 text-sm font-black text-slate-800"><span class="text-slate-400">▾</span><span>{{ $accountClasses[$classKey][0] }}</span><span class="ml-auto rounded-full bg-white px-2 py-0.5 text-[10px] text-slate-500 ring-1 ring-slate-200">{{ $accounts->where('account_class', $classKey)->count() }}</span></div>
            ...
            <form id="account-editor" wire:submit="{{ $editingAccountId ? 'saveAccount' : 'addAccount' }}" class="h-fit rounded-xl border border-slate-200 bg-white p-5 xl:sticky xl:top-24">
                <div class="mb-5 border-b border-slate-100 pb-4"><p class="text-[10px] font-black uppercase tracking-wider text-amber-700">Account setup</p><h3 class="mt-1 text-base font-black text-slate-900">{{ $editingAccountId ? 'Update selected account' : 'Add a new account' }}</h3></div>
                <div class="space-y-4">
                    <label class="grid items-center gap-2 sm:grid-cols-[120px_1fr]"><span class="text-xs font-bold text-slate-700">Account</span><input wire:model="accountCode" placeholder="Account code" class="w-full rounded-lg border-slate-300 bg-white px-3 py-2.5 font-mono text-sm font-bold focus:border-amber-400 focus:ring-amber-400"></label>
                    <label class="grid items-center gap-2 sm:grid-cols-[120px_1fr]"><span class="text-xs font-bold text-slate-700">Account name</span><input wire:model="accountName" placeholder="Account name" class="w-full rounded-lg border-slate-300 bg-white px-3 py-2.5 text-sm font-semibold focus:border-amber-400 focus:ring-amber-400"></label>
                    ...
                    <label class="grid items-center gap-2 sm:grid-cols-[120px_1fr]"><span class="text-xs font-bold text-slate-700">Parent account</span><select wire:model="parentId" class="w-full rounded-lg border-slate-300 bg-white px-3 py-2.5 text-xs font-semibold focus:border-amber-400 focus:ring-amber-400"><option value="">Select account…</option>@foreach($accounts as $account)<option value="{{ $account->id }}">{{ $account->code }} · {{ $account->name }}</option>@endforeach</select></label>
                    <label class="grid items-center gap-2 sm:grid-cols-[120px_1fr]"><span class="text-xs font-bold text-slate-700">Currency</span><input value="{{ $currency }}" disabled class="w-full rounded-lg border-slate-200 bg-slate-50 px-3 py-2.5 text-sm font-bold text-slate-500"></label>
                    ...
                    <div class="flex flex-wrap gap-2 border-t border-slate-100 pt-4"><button class="rounded-lg bg-amber-400 px-4 py-2.5 text-xs font-black text-slate-950 hover:bg-amber-300">{{ $editingAccountId ? 'Update' : 'Add' }}</button>@if($editingAccountId && !$editingAccountIsSystem)<button type="button" wire:click="deleteAccount" wire:confirm="Delete this unused ledger account permanently?" class="rounded-lg border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-black text-rose-700">Delete</button>@endif<button type="button" wire:click="cancelAccountEdit" class="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-xs font-black text-slate-700 hover:bg-slate-50">Clear</button></div>
                    @if($editingAccountId && $editingAccountIsSystem)<div class="rounded-lg border border-slate-200 bg-slate-50 p-3"><p class="text-xs font-semibold leading-5 text-slate-600">Protected system ledger: editing is allowed, but deletion is blocked.</p></div>@endif
                </div>
            </form>
            </div>
        </section>
    ...
    @if($tab==='journals')
        ...
        @endif
        <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200"><div class="flex flex-wrap items-end gap-3"><div><h2 class="font-black">Journal register</h2><p class="text-sm text-slate-500">Posted journals are immutable. Corrections use reversals.</p></div><label class="ml-auto text-xs font-bold">Rejection / reversal reason<input wire:model="actionReason" class="mt-1 rounded-xl border-slate-300 text-sm" placeholder="Required for reversal"></label></div><div class="mt-4 overflow-x-auto"><table class="min-w-full text-sm"><thead class="text-left text-xs uppercase text-slate-500"><tr><th class="py-2">Number</th><th>Date</th><th>Narration</th><th>Status</th><th>Debit</th><th>Actions</th></tr></thead><tbody>@foreach($journals as $journal)<tr class="border-t"><td class="py-3 font-bold">{{ $journal->number }}</td><td>{{ $journal->journal_date->format('d M Y') }}</td><td>{{ $journal->description }}</td><td><span class="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{{ ucfirst($journal->status) }}</span></td><td>{{ $currency }} {{ number_format($journal->lines->sum('debit'),2) }}</td><td><div class="flex gap-2 text-xs font-bold">@if($journal->status==='draft' && auth()->user()->hasPermission('accounting.journals.submit'))<button wire:click="submitJournal({{ $journal->id }})">Submit</button>@elseif($journal->status==='submitted' && auth()->user()->hasPermission('accounting.journals.approve'))<button wire:click="approveJournal({{ $journal->id }})">Approve</button>@elseif($journal->status==='approved' && auth()->user()->hasPermission('accounting.journals.post'))<button wire:click="postJournal({{ $journal->id }})">Post</button>@elseif($journal->status==='posted' && auth()->user()->hasPermission('accounting.journals.reverse'))<button class="text-red-700" wire:click="reverseJournal({{ $journal->id }})">Reverse</button>@endif</div></td></tr>@endforeach</tbody></table></div><div class="mt-4">{{ $journals->links() }}</div></div>
    @endif
