<div>
    <div class="mb-8">
        <h1 class="text-2xl font-semibold text-darken">Student Categories</h1>
        <p class="text-gray-500 text-sm mt-1">Categories like Day, Boarding, or Special determine which fee structure a student is automatically mapped to.</p>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 h-fit">
            <h2 class="font-semibold text-darken mb-4">Add a category</h2>
            <form wire:submit="add" class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Name</label>
                    <input type="text" wire:model="name" placeholder="e.g. Day, Boarding, Special"
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <h2 class="font-semibold text-darken mb-4">Existing categories</h2>

            @if($categories->isEmpty())
                <p class="text-gray-400 text-sm">No categories yet — add your first one on the left.</p>
            @else
                <div class="space-y-2">
                    @foreach($categories as $category)
                        <div class="flex items-center justify-between py-3 px-3 rounded-lg border border-gray-100">
                            <div>
                                <p class="font-medium text-darken text-sm">{{ $category->name }}</p>
                                <p class="text-xs text-gray-400">{{ $category->students_count }} student{{ $category->students_count === 1 ? '' : 's' }} assigned</p>
                            </div>

                            @if($deletingId === $category->id)
                                <div class="flex items-center space-x-2">
                                    <span class="text-xs text-gray-500">Delete this category?</span>
                                    <button wire:click="delete({{ $category->id }})" class="text-xs bg-red-500 text-white px-3 py-1.5 rounded-full hover:bg-red-600">Yes</button>
                            ...
                            @else
                                <button wire:click="confirmDelete({{ $category->id }})" class="text-gray-400 hover:text-red-500 transition-colors">
