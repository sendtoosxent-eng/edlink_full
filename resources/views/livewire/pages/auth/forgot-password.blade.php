...
new #[Layout('layouts.guest')] class extends Component
{
    ...
    public function sendResetLink(): void
    {
        ...
        if ($school) {
            // Using the broker's complete flow enforces token throttling and
            // resolves the account using both its school and email address.
        ...
        session()->flash('status','If those details match an Edlink account, a reset link has been sent by email.');
    }
}; ?>
...
<div>
    ...
    <form wire:submit='sendResetLink' class='mt-6 space-y-4'>
        <div><label class='text-sm font-medium'>School number</label><input wire:model='school_number' required autocomplete='organization' class='mt-1 w-full rounded-lg border-gray-300 uppercase' placeholder='EDL-4K9P2'>@error('school_number')<span class='text-xs text-red-600'>{{ $message }}</span>@enderror</div>
